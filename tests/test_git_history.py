...
def make_repo(tmpdir):
    ...
    sub_dir = repo_dir / "dir"
    sub_dir.mkdir()
    (sub_dir / "items.json").write_text(
        json.dumps(
            [
                {
                    "product_id": 1,
...
@pytest.mark.parametrize("namespace", (None, "custom"))
def test_file_in_subdirectory(repo, tmpdir, namespace):
    runner = CliRunner()
    db_path = str(tmpdir / "db.db")
    with runner.isolated_filesystem():
        options = ["file", db_path, str(repo / "dir/items.json"), "--repo", str(repo)]
        if namespace:
            options += ["--namespace", namespace]
        result = runner.invoke(cli, options)
    assert result.exit_code == 0
